...
namespace Sim.Sec.Desenvolvimento.ComercioAmbulante.ViewModel
{
    ...
    public class vmEstatisticas : NotifyProperty
    {
        #region Declarations
        ...
        #region Constructor
        public vmEstatisticas()
        {
            GlobalNavigation.Pagina = "ESTATISTICAS";
        ...
        private void AsyncListarAtendimento(List<string> sqlcommand)
        {
            ...
            Task.Factory.StartNew(() => mdatar.ComercioAmbulante(mdatacm.RCAmbulantes(sqlcommand)))
                .ContinueWith(task =>
                {
                    if (task.IsCompleted)
                    {
        ...
        private void AsyncFlowDoc()
        {
            ...
            new System.Threading.Thread(() =>
            {
                ...
                FlowDoc.Dispatcher.BeginInvoke(System.Windows.Threading.DispatcherPriority.Background,
                    new Action(delegate () {
                        FlowDoc = PreviewInTable(new mDataReport().ComercioAmbulante(mdatacm.RCAmbulantes(Parametros())));
                        //FlowDoc = PreviwChart(Chart(new mDataReport().ComercioAmbulante(mdatacm.RCAmbulantes(Parametros()))));
                    }));
            ...
            Task.Factory.StartNew(() => mdatar.ComercioAmbulante(mdatacm.RCAmbulantes(Parametros()))).ContinueWith(task =>
            {
