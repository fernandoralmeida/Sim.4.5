...
namespace Sim.Sec.Desenvolvimento.Shared.ViewModel.Empresa
{
    ...
    class vmReport : NotifyProperty
    {
        ...
        private mReportEmpresas mReport = new mReportEmpresas();
        private vmCharts  ProvedorGrafico = new vmCharts();
        private ObservableCollection<mPJ_Ext> _listaat = new ObservableCollection<mPJ_Ext>();
        ...
        public ObservableCollection<mPJ_Ext> ListarAtendimentos
        {
            get { return _listaat; }
            set
            {
        ...
        {
            //AsyncCNAE();
            GlobalNavigation.Pagina = "RELATÓRIO EMPRESAS";
            DataI = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            DataF = DateTime.Now;
            BlackBox = Visibility.Collapsed;
            PrintBox = Visibility.Collapsed;
            MainBox = Visibility.Visible;
        ...
        private void AsyncListarEmpresas(List<object> sqlcommand)
        {
            ...
            Task.Factory.StartNew(() => mdata.R_Empresas(sqlcommand))
                .ContinueWith(task =>
                {
                    if (task.IsCompleted)
                    {
                        mReport.Clear();
                        mReport.Show(task.Result, sqlcommand);
                        mReport.Periodo.Add(DataI.ToShortDateString());
                        mReport.Periodo.Add(DataF.ToShortDateString());
                        Charts = ProvedorGrafico.Empresas(mReport, true);
                        BlackBox = Visibility.Collapsed;
                        StartProgress = false;
                    }
                    else
        ...
        private void AsyncFlowDoc()
        {
            ...
            Task.Factory.StartNew(() => mdata.R_Empresas(Parametros())).ContinueWith(task =>
            {
                ...
                {
                    mReport.Clear();
                    mReport.Show(task.Result, Parametros());
                    BlackBox = Visibility.Collapsed;
                    MainBox = Visibility.Collapsed;
                    PrintBox = Visibility.Visible;
                    StartProgress = false;
        ...
        private FlowDocument PreviewInTable(mReportEmpresas obj)
        {
            try
            {
                ...
                foreach (KeyValuePair<string, int> x in obj.Porte)
                {
                ...
                pheader.Cells.Add(new TableCell(new Paragraph(new Run(string.Format("PERFIL DE EMPRESAS ATENDIDAS ENTRE {0} {1}", Parametros()[0], Parametros()[1]))) { Padding = new Thickness(5) }) { ColumnSpan = obj.Porte.Count, BorderThickness = new Thickness(0.5), BorderBrush = Brushes.Black });
                ...
                hcnae.Cells.Add(new TableCell(new Paragraph(new Bold(new Run(string.Format("USO LOCAL")))) { Padding = new Thickness(5) }) { ColumnSpan = 2, BorderThickness = new Thickness(0.5), BorderBrush = Brushes.Black });
                ...
                if (obj != null)
                {
                    ...
                    foreach (KeyValuePair<string, int> x in obj.UsoLocal)
                    {
                        TableRow row = new TableRow();
                        rg1.Foreground = Brushes.Black;
                        rg1.Rows.Add(row);
                ...
                tbairro.Cells.Add(new TableCell(new Paragraph(new Bold(new Run(string.Format("SETOR DE ATUAÇÃO")))) { Padding = new Thickness(5) }) { ColumnSpan = 2, BorderThickness = new Thickness(0.5), BorderBrush = Brushes.Black });
                ...
                TableRow tatv = new TableRow();
                tatv.Cells.Add(new TableCell(new Paragraph(new Bold(new Run(string.Format("ATIVIDADES RELACIONADAS")))) { Padding = new Thickness(5) }) { ColumnSpan = 2, BorderThickness = new Thickness(0.5), BorderBrush = Brushes.Black });
